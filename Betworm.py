...
###############################################################################################################
## [Title]: BitWorm.py -- a BitWrom Was Here :()
## [Author]: Yehia Mamdouh (B^^H) -- @yehiamamdouh1
##-------------------------------------------------------------------------------------------------------------   
## [Thanks] to , Mike,  Mohamed]:
## Feel free to modify and/or distribute this script as you wish.
###############################################################################################################
...
print "detect Active Connections"
print ""

import subprocess
import re

p = subprocess.Popen("netstat -ntu | awk '{print $5}' | cut -d: -f1 | sort | uniq -c | sort -n | tee /tmp/active.txt", stdout=subprocess.PIPE, shell=True)
(output, err) = p.communicate()
p_status=p.wait()

try:
	file = open('/tmp/active.txt', "r")
	ips = []
	...
	for ip in ips:
		...
		if addy is not '':
			print (addy)
			f2 = open('/tmp/check.txt', 'a+')
			f2.writelines(addy + '\n')
			f2.close()
except IOError, (errno, strerror):
	print "I/O Error(%s) : %s" % (errno, strerror)

#Download last Malware Domain List 

url = 'http://www.malwaredomainlist.com/hostslist/ip.txt'

print ""
print "Checking malicious connections"
f = urllib2.urlopen(url)
data = f.read()
with open("/tmp/code2.txt", "wb") as code:
    code.write(data)
    f1 = open("/tmp/check.txt")
    f2 = open("/tmp/code2.txt")
    line1 = next(f1)
    line2 = next(f2)
...
#Betworm Self Deleted
os.remove("/tmp/Betworm.py")
os.remove("/tmp/active.txt")
os.remove("/tmp/check.txt")
os.remove("/tmp/code2.txt")
